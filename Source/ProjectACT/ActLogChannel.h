...
#define GITHASH_TCHAR (ANSI_TO_TCHAR(GITHASH))

// git hash, 함수명과 라인 번호만 출력하는 간단한 로그
#define ACT_LOG(LogCategory, Verbosity, Format, ...) \
	UE_LOG(LogCategory, Verbosity, TEXT("[(%s)%s::%d] %s"), GITHASH_TCHAR, FUNCTION_TCHAR, __LINE__, *FString::Printf(Format, ##__VA_ARGS__))

// 파일이름(또는 파일 경로)까지 출력하는 전체 로그  
#define ACT_FULL_LOG(LogCategory, Verbosity, Format, ...) \
	UE_LOG(LogCategory, Verbosity, TEXT("[(%s)%s::%s::%d] %s"), GITHASH_TCHAR, FILE_TCHAR, FUNCTION_TCHAR, __LINE__, *FString::Printf(Format, ##__VA_ARGS__))
